...
線上版**僅供瀏覽介面**。本專案的資料存取依賴自架的檔案伺服器 API（`/api/file/*`），GitHub Pages 是純靜態託管、不提供該 API，因此線上版無法新增或儲存書評，開啟時會出現讀取失敗——這是預期行為，不是故障。要實際使用請依下方「安裝」自架。
...
- **書庫與搜尋** — 整版書單支援排序與多重篩選；搜尋支援 `書名`／`作者`／`#標籤`／`@人名`
