...
| 項目 | 值 |
|---|---|
| 開發分支 | `wasteland-ol`（orphan，樹裡只有廢土；**本分支根＝專案根**） |
| 整合分支 | `wasteland-ol-release`（衍生物，永不手改，只為開 PR 上線） |
| 上線後路徑 | main 的 `wasteland-ol/` 子資料夾，與 `futaba-bookshelf/` 平行 |
...
1. **GitHub Pages**：開發推 `wasteland-ol` → 同步到 `wasteland-ol-release` → 開草稿 PR →
   **由 Ting 親自合併進 `main`** 才會更新線上版。
   **AI 不得直推 main、不得自行按合併**（Ting 2026-08 定案）。
   完整同步指令與推送前必驗見 `AI-CONTEXT.md` §4。
