...
2. **合併走 PR。誰來按合併，兩個專案規則不同——不要弄混：**

   | 專案 | 合併權 |
   |---|---|
   | 雙葉書庫 `futaba-bookshelf` | **由 Ting 親自按**（2026-08 鎖定決策）。AI 只推分支＋開草稿 PR，不得自行合併。 |
   | 廢土 `wasteland-ol` | **AI 推完即自動合併**（Ting 2026-08-10 明確指示，推翻前述限制，**僅限廢土**）。 |

   廢土的自動合併流程：推分支 → 確保 PR 存在且改為 ready → 合併進 main → 線上版更新。
   仍然走 PR 是為了留下可回溯的 diff 紀錄，不是直推 main。
3. **只碰自己專案的資料夾。**
